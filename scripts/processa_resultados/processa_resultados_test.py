...
def test_get_first_role_request():
    assert get_first_role_request(
        slave) == "    1  20.699564 192.168.2.12 → 192.168.2.10 OpenFlow 90 Type: OFPT_ROLE_REQUEST\n"


def test_get_first_role_reply():
    assert get_first_role_reply(
        slave) == "    1  20.699761 192.168.2.10 → 192.168.2.12 OpenFlow 90 Type: OFPT_ROLE_REPLY\n"


def test_get_first_multipart_request():
    assert get_first_multipart_request(
        slave) == "    1  20.704851 192.168.2.12 → 192.168.2.10 OpenFlow 82 Type: OFPT_MULTIPART_REQUEST\n"


def test_get_first_multipart_reply():
    assert get_first_multipart_reply(
        slave) == "    1  20.705079 192.168.2.10 → 192.168.2.12 OpenFlow 370 Type: OFPT_MULTIPART_REPLY\n"


def test_get_first_packet_out():
    assert get_first_packet_out(
        slave) == "    1  20.787313 192.168.2.12 → 192.168.2.10 OpenFlow 193 Type: OFPT_PACKET_OUT\n"
